...
def plot_vs_zenith(y, data, cuts, **kwargs):
    zenith_bins = np.linspace(0.75,1.0,21)
    # energy_bins = np.linspace(
        # energy_bins.min(), energy_bins.max(), kwargs['numbins'] + 1)
...
def energyres(data, cuts, **opts):
    ...
    opts['ylabel'] = '$\log_{10}(E_{LLH}/E_{\mathrm{true}})$'
    ...
    # energy_bins = get_energy_bins()
    # energy_bins = np.linspace(
    #     energy_bins.min(), energy_bins.max(), opts['numbins'] + 1)
    #
    # bin_centers, bin_medians, error = get_medians(
    #     np.log10(energyMC), energy_res, energy_bins)
    #
    # plt.errorbar(bin_centers, bin_medians, yerr=error, marker='.')
    # plt.xlabel('$\log_{10}(E_{\mathrm{true}}/\mathrm{GeV})$')
    ...
    # plt.title(r'ShowerLLH - IT73 - {} LLH bins'.format(opts['bintype']))
    # # plt.xlim([5.5, 9.5])
    # outfile = opts['outdir'] + 'energy_res_{}.png'.format(opts['bintype'])
    # plt.savefig(outfile)
    # plt.close()
...
def eres_position(data, cuts, **opts):
    ...
    plt.ylabel('$\log_{10}(E_{LLH}/E_{\mathrm{true}})$')
    plt.title(r'ShowerLLH - IT73 - {} LLH bins'.format(opts['bintype']))
    outfile = opts['outdir'] + 'energy_res_vs_pos_{}.png'.format(opts['bintype'])
    plt.savefig(outfile)
    plt.close()
...
def coreres(data, cuts, **opts):
    ...
    #
    # bin_centers, bin_medians, error = get_medians(
    #     np.log10(energyMC), core_res, energy_bins)
    #
    # plt.errorbar(bin_centers, bin_medians, yerr=error, marker='.', )
    # plt.xlabel('$\log_{10}(E_{\mathrm{true}}/\mathrm{GeV})$')
    ...
    # # plt.xlim([5.5, 9.5])
    # plt.ylim([0, 90])
    # outfile = opts['outdir'] + 'core_res_{}.png'.format(opts['bintype'])
    # plt.savefig(outfile)
    # plt.close()
...
def LLHenergy_MCenergy(data, cuts, **opts):
    ...
    plt.xlim([4, 9.5])
    plt.ylim([5, 9.5])
    cb = plt.colorbar(label='$\log_{10}{P(E_{LLH}|E_{\mathrm{true}})}$')
...
def effarea(data, cuts, **opts):
    ...
    plt.title(r'ShowerLLH - IT73 - {} LLH bins'.format(opts['bintype']))
    # plt.xlim([5.5, 9.5])
    outfile = opts['outdir'] + 'effarea_vs_energy_{}.png'.format(opts['bintype'])
    plt.savefig(outfile)
    plt.close()
...
if __name__ == "__main__":
    ...
    p.add_argument('-n', '--numbins', dest='numbins', type=float,
                   default=20, help='Number of energy bins')
    args = p.parse_args()
    opts = vars(args).copy()
